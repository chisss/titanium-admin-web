<template>
  ...
  <div class="ti-page">
    <div class="ti-card" v-loading="loading">
      <TiDetailHeader :title="`账单详情 - ${bill?.billId ?? ''}`">
        <template #meta>
          <TiStatusTag v-if="bill" :value="bill.status" :label="billStatusLabel(bill.status)" />
        </template>
      </TiDetailHeader>

      <el-descriptions v-if="bill" :column="detailColumns" border style="margin-bottom: 24px">
        <!-- 🔴 账单号即主键 billId：此前读遗留字段 billNo 恒 '-'（D-501-47） -->
        <el-descriptions-item label="账单号">{{ bill.billId || '-' }}</el-descriptions-item>
        <el-descriptions-item label="账单金额">{{ formatAmount(bill.amount) }}</el-descriptions-item>
      ...
      <template v-else-if="pricingFacts">
        <el-descriptions :column="reconciliationColumns" border class="reconciliation-summary">
        ...
        <el-table :data="pricingFacts.commissionPayables" border size="small">
          ...
          <el-table-column label="基数" width="125" align="right"><template #default="{ row }">{{ internalAmountText(row.baseAmount, row.currency) }}</template></el-table-column>
          <el-table-column label="分润比例" width="105"><template #default="{ row }">{{ internalRateText(row.splitRate) }}</template></el-table-column>
          <el-table-column label="应付金额" width="130" align="right"><template #default="{ row }">{{ internalAmountText(row.payableAmount, row.currency) }}</template></el-table-column>
...
/** 账单概览：字段短的标签-值对，宽屏 3 档 */
const detailColumns = useDetailColumns(3)
/** 勾稽摘要：4 项税额短值并排的语义面板，宽屏 4 档 */
const reconciliationColumns = useDetailColumns(4)
...
.subsection-title { margin: 18px 0 10px; color: $text-primary; font-size: 14px; font-weight: 600; }
